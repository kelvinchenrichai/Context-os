...
export default function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleGoogleLogin = () => {
    // Guard against double/triple clicks firing overlapping OAuth round-trips
    // while the full-page redirect is still in flight.
    if (googleLoginStarted) return;
    setGoogleLoginStarted(true);

    const loginUrl = 'https://context-os-api.kelvinchenrichai.workers.dev/auth/google/start';

    if (isIosPwa()) {
      // In iOS PWA standalone mode, OAuth redirects don't work well in the same context
      // Show a hint to open in Safari first
      setShowIosHint(true);
    ...
    // Normal browser — use assign for cleaner navigation
    ...
    <div className="min-h-screen bg-stone-50 dark:bg-stone-950 flex items-center justify-center px-4">
      <div className="w-full max-w-sm space-y-8">
        ...
        <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-2xl p-8 space-y-4 shadow-sm">
          ...
          {/* iOS PWA hint */}
          {showIosHint && (
            <div className="text-xs text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 rounded-lg py-2.5 px-3 leading-relaxed">
              💡 iOS 提示：如果登入沒有跳轉，請用 Safari 瀏覽器打開{' '}
              <a
                href="https://context-os-12u.pages.dev"
                className="underline font-semibold"
                target="_blank"
                rel="noopener noreferrer"
              >
                context-os-12u.pages.dev
              </a>{' '}
              來登入，登入後再加入主畫面。
            </div>
          )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-2.5 px-4 border border-stone-200 dark:border-stone-700 rounded-xl text-sm font-sans font-medium text-stone-700 dark:text-stone-300 hover:bg-stone-50 dark:hover:bg-stone-800 transition-colors active:scale-95 disabled:opacity-50"
          >
        ...
        <div className="text-center">
          <p className="text-[11px] text-stone-400 dark:text-stone-500 font-sans leading-relaxed">
            📱 iOS 用戶：請先在 Safari 登入，<br />再從「分享」→「加入主畫面」安裝 App
          </p>
        </div>
